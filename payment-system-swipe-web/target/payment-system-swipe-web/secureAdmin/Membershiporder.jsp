<%-- 
    Document   : Membershiporder
    Created on : Apr 10, 2017, 3:02:31 PM
    Author     : Billy
--%>
...
<hr class="noscreen" />
	...
	<div id="content-box">
        ...
        <form name="Members" method="post" action="../Payments_Report_All">
            ...
            <c:choose>
                <c:when test="${empty param.startdate}">
                    <center><input style="visibility:hidden" type="text" name="startdate" value = ""/><br/></center>
                </c:when>
                <c:otherwise>
                    <center><input style="visibility:hidden" type="text" name="startdate" value = "<c:out value="${param.startdate}"/>"/><br/></center>
                </c:otherwise>
            </c:choose> 
            <c:choose>
                <c:when test="${empty param.enddate}">
                    <center><input style="visibility:hidden" type="text" name="enddate" value = ""/><br/></center>
                </c:when>
                <c:otherwise>
                    <center><input style="visibility:hidden" type="text" name="enddate" value = "<c:out value="${param.enddate}"/>"/><br/></center>
                </c:otherwise>
            </c:choose>       
          <script>         
              submitform();     
