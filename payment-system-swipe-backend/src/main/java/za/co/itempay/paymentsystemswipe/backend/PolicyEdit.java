 ...
 */
@WebServlet(name = "PolicyEdit", urlPatterns = {"/PolicyEdit"})
public class PolicyEdit extends HttpServlet {


    static Logger logger = Logger.getLogger(PolicyEdit.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logger.info("PolicyEdit accessed by: " + User);
            ...
            try {
                ...
                date = new Date();
                logger.debug(dateFormat.format(date));
                logger.debug("Query: update T_BP_Customer_Policies set CoverAmount = " + TestAmount + ", AdminFee =  " + TestFee + ", CoverType = '" + SelectedType + "', PaymentMethod = '" + SelectedPayment + "' where BP_Cust_ID = " + BP_Cust_ID);
                stinsert.executeUpdate("update T_BP_Customer_Policies set CoverAmount = " + TestAmount + ", AdminFee =  " + TestFee + ", CoverType = '" + SelectedType + "', PaymentMethod = '" + SelectedPayment + "' where BP_Cust_ID = " + BP_Cust_ID);
            } catch (SQLException ex) {
                date = new Date();
                logger.fatal(dateFormat.format(date));
                logger.fatal("SQLException " + ex.getMessage());
                logger.fatal("With query: update T_BP_Customer_Policies set CoverAmount = " + TestAmount + ", AdminFee =  " + TestFee + ", CoverType = '" + SelectedType + "', PaymentMethod = '" + SelectedPayment + "' where BP_Cust_ID = " + BP_Cust_ID);
            }
            ...
            if (SelectedType.equals("Normal")) {
                try {
                    ...
                    date = new Date();
                    logger.debug(dateFormat.format(date));
                    logger.debug("Query: select count(*) from T_BP_Customer_Dependants where BP_Cust_ID = " + BP_Cust_ID);
                    ResultSet rsselect = stselect.executeQuery("select count(*) from T_BP_Customer_Dependants where BP_Cust_ID = " + BP_Cust_ID);
                    while (rsselect.next()) {
                        String NoDependants = rsselect.getString(1);
                        date = new Date();
                        logger.debug(dateFormat.format(date));
                        logger.debug("NoDependants: " + NoDependants);
                        if (Integer.parseInt(NoDependants) > 0) {
                            try {

                                out.println("<center><table border=\"1\">");
                                out.println("<tr>");
                                out.println("<th>Name</th>");
                                out.println("<th>Identity Number</th>");
                                out.println("<th>Dependant Number</th>");
                                out.println("</tr>");


                                Statement stDependantDetails = Utils.TGDBcon.createStatement();
                                date = new Date();
                                logger.debug(dateFormat.format(date));
                                logger.debug("Query: select FirstName + ' ' + SurName, Identity_No, Member_No from T_BP_Customer_Dependants where BP_Cust_ID = " + BP_Cust_ID);
                                ResultSet rsDependantDetails = stDependantDetails.executeQuery("select FirstName + ' ' + SurName, Identity_No, Member_No from T_BP_Customer_Dependants where BP_Cust_ID = " + BP_Cust_ID);
                                while (rsDependantDetails.next()) {
                                    String Name = rsDependantDetails.getString(1);
                                    date = new Date();
                                    logger.debug(dateFormat.format(date));
                                    logger.debug("Name: " + Name);
                                    String ID = rsDependantDetails.getString(2);
                                    logger.debug("ID: " + ID);
                                    String DepNo = rsDependantDetails.getString(3);
                                    logger.debug("DepNo: " + DepNo);
                                    out.println("<tr>");
                                    String FormattedID = ID.replaceAll(" ", "+");
                                    out.println("<td><a href=\"/PaymentSystemSwipe/secureAdmin/DependantEdit.jsp?id=" + FormattedID + "\">" + Name + "</a></td>");
                                    out.println("<td><a href=\"/PaymentSystemSwipe/secureAdmin/DependantEdit.jsp?id=" + FormattedID + "\">" + ID + "</a></td>");
                                    out.println("<td><a href=\"/PaymentSystemSwipe/secureAdmin/DependantEdit.jsp?id=" + FormattedID + "\">" + DepNo + "</a></td>");
                                    out.println("</tr>");
                                }
                                ...
                                logger.fatal(dateFormat.format(date));
                                logger.fatal("SQLException " + ex.getMessage());
                                logger.fatal("With query: select FirstName + ' ' + SurName, Identity_No, Member_No from T_BP_Customer_Dependants where BP_Cust_ID = " + BP_Cust_ID);
                            }
                        }
                    }
                } catch (SQLException ex) {
                    ...
                    logger.fatal(dateFormat.format(date));
                    logger.fatal("SQLException " + ex.getMessage());
                    logger.fatal("With query: select count(*) from T_BP_Customer_Dependants where BP_Cust_ID = " + BP_Cust_ID);
                }
