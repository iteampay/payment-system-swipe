...
@WebServlet(name = "SettlementCalculator", urlPatterns = {"/SettlementCalculator"})
public class SettlementCalculator extends HttpServlet {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    ResultSet rsselect = stselect.executeQuery("Query:SELECT SUM(A.Amount)FROM T_EasyPay_Messages A, T_BP_Beneficiaries B WHERE a.Settled = 'N' AND a.beneficiary_no = b.beneficiary_no AND A.Deleted = 'N' AND B.Split = 1  ");

                    while (rsselect.next()) {
                        TotalUnsettled = rsselect.getString(1);
                        date = new Date();
                        logger.debug(dateFormat.format(date));
                        logger.debug("Total Unsettled: " + TotalUnsettled);
                    }


                    //need to change to a correct no
                    logger.debug("Query:SELECT count(*) FROM T_EasyPay_Messages A, T_BP_Beneficiaries B WHERE a.Settled = 'N' AND a.beneficiary_no = b.beneficiary_no AND A.Deleted = 'N' AND B.Split = 1");
                    rsselect = stselect.executeQuery("Query:SELECT count(*) FROM T_EasyPay_Messages A, T_BP_Beneficiaries B WHERE a.Settled = 'N' AND a.beneficiary_no = b.beneficiary_no AND A.Deleted = 'N' AND B.Split = 1");
                    ...
                    logger.debug("Query:SELECT SUM(A.TP_Fee)FROM T_EasyPay_Messages A, T_BP_Beneficiaries B WHERE a.Settled = 'N' AND a.beneficiary_no = b.beneficiary_no AND A.Deleted = 'N' AND B.Split = 1 ");
                    rsselect = stselect.executeQuery("Query:SELECT SUM(A.TP_Fee)FROM T_EasyPay_Messages A, T_BP_Beneficiaries B WHERE a.Settled = 'N' AND a.beneficiary_no = b.beneficiary_no AND A.Deleted = 'N' AND B.Split = 1  ");
                    ...
                    Balance = String.valueOf(Double.parseDouble(TotalUnsettled) - Double.parseDouble(TotalTPFee));
                    date = new Date();
                    logger.debug(dateFormat.format(date));
                ...
                try {
                    ...
                    date = new Date();
                    logger.debug(dateFormat.format(date));
                    logger.debug("Query:SELECT DATENAME(DAY,a.File_Date_Time), DATENAME(MONTH,a.File_Date_Time),DATENAME(YEAR,a.File_Date_Time), b.Beneficiary_Name, a.Beneficiary_no, sum(a.Amount), sum(A.TP_Fee)FROM T_EasyPay_Messages A, T_BP_Beneficiaries B WHERE a.Settled = 'N'AND a.beneficiary_no = b.beneficiary_no AND A.Deleted = 'N'AND B.Split = 1 GROUP by DATENAME(DAY,a.File_Date_Time), DATENAME(MONTH,a.File_Date_Time),DATENAME(YEAR,a.File_Date_Time), b.Beneficiary_Name, a.Beneficiary_no ORDER by DATENAME(DAY,a.File_Date_Time), DATENAME(MONTH,a.File_Date_Time),DATENAME(YEAR,a.File_Date_Time), b.Beneficiary_Name");
                    ...
                    ResultSet rsselect = stselect.executeQuery("Query:SELECT DATENAME(DAY,a.File_Date_Time), DATENAME(MONTH,a.File_Date_Time),DATENAME(YEAR,a.File_Date_Time), b.Beneficiary_Name, a.Beneficiary_no, sum(a.Amount), sum(A.TP_Fee)FROM T_EasyPay_Messages A, T_BP_Beneficiaries B WHERE a.Settled = 'N'AND a.beneficiary_no = b.beneficiary_no AND A.Deleted = 'N'AND B.Split = 1 GROUP by DATENAME(DAY,a.File_Date_Time), DATENAME(MONTH,a.File_Date_Time),DATENAME(YEAR,a.File_Date_Time), b.Beneficiary_Name, a.Beneficiary_no ORDER by DATENAME(DAY,a.File_Date_Time), DATENAME(MONTH,a.File_Date_Time),DATENAME(YEAR,a.File_Date_Time), b.Beneficiary_Name ");
...
//AND D.Deleted = 'N'       ");         
                    ...
                    out.println("					<h3>Settlement Calculator</h3>");
...
//outfile.write("Payment report from: " + StartDate + " to: " + EndDate + "\r\n");
                    ...
                    out.println("<tr>");
                    out.println("<td>Total Unsettled</td>");
                    outfile.write("Total Unsettled,");
                    out.println("<td>" + TotalUnsettled + "</td>");
                    outfile.write(TotalUnsettled + ",");
                    out.println("<td>" + TotalPaymentsNumber + "</td>");
                    outfile.write(TotalPaymentsNumber + "\r\n");
                    out.println("</tr>");

                    out.println("<tr>");
                    out.println("<td>Total Touchpay Fee on unsettled</td>");
                    outfile.write("Total Touchpay Fee on unsettled,");
                    ...
                    out.println("<tr>");
                    out.println("<td>Unsettled Balance</td>");
                    outfile.write("Unsettled Balance,");
                    ...
                    out.println("<tr>");
                    out.println("<td>Unsettled VAT</td>");
                    outfile.write("Unsettled VAT,");
                    out.println("<td>" + VAT + "</td>");
                    outfile.write(VAT + ",");
                    out.println("</tr>");

                    out.println("<tr>");
                    out.println("<td>Total Unsettled amount</td>");
                    outfile.write("Total Unsettled amount,");
...
//outfile.write("Balance\r\n");
                    ...
                    while (rsselect.next()) {
                        File_Date_Time = rsselect.getString(1) + rsselect.getString(2) + rsselect.getString(3);
                        Customer_ID = rsselect.getString(4);
