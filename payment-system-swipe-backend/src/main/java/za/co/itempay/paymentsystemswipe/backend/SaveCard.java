...
@WebServlet(name = "Save_Card", urlPatterns = {"/Save_Card"})
public class SaveCard extends HttpServlet {


    static Logger logger = Logger.getLogger(SaveCard.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logger.info("Save_Card accessed by: " + User);
            ...
            try {
                ...
                date = new Date();
                logger.debug(dateFormat.format(date));
                logger.debug("Query:select top 1 PAN, EasyPay_No, Barcode_Info from T_EasyPay_No_Track_New where Status = 'Available' and PAN = '" + PAN + "'");
                ResultSet rsselect = stselect.executeQuery("select top 1 PAN, EasyPay_No, Barcode_Info from T_EasyPay_No_Track_New where Status = 'Available' and PAN = '" + PAN + "'");
                while (rsselect.next()) {
                ...
                logger.fatal(dateFormat.format(date));
                logger.fatal("SQLException " + ex.getMessage());
                logger.fatal("Query:select top 1 PAN, EasyPay_No, Barcode_Info from T_EasyPay_No_Track_New where Status = 'Available' and PAN = '" + PAN + "'");
            ...
            try {
                ...
                date = new Date();
                logger.debug(dateFormat.format(date));
                logger.debug("Query: insert into T_BP_Card_Beneficiaries (PAN, Beneficiary_No, Acct_Ref_No, Date_Added, Editable) values ('" + PAN + "', '" + Beneficiary_No + "', '" + Member_No + "', getdate(), 'T')");
                stinsert.executeUpdate("insert into T_BP_Card_Beneficiaries (PAN, Beneficiary_No, Acct_Ref_No, Date_Added, Editable) values ('" + PAN + "', '" + Beneficiary_No + "', '" + Member_No + "', getdate(), 'T')");
            } catch (SQLException ex) {
                date = new Date();
                logger.fatal(dateFormat.format(date));
                logger.fatal("SQLException " + ex.getMessage());
                logger.fatal("With query: insert into T_BP_Card_Beneficiaries (PAN, Beneficiary_No, Acct_Ref_No, Date_Added, Editable) values ('" + PAN + "', '" + Beneficiary_No + "', '" + Member_No + "', getdate(), 'T')");
                String TGResponseMessage = "Database Error";
            ...
            try {
                ...
                date = new Date();
                logger.debug(dateFormat.format(date));
                logger.debug("Query: insert into T_EasyPay_No_Track (PAN, EasyPay_No, Barcode_Info) values ('" + PAN + "', '" + EasyPay_No + "', '" + Barcode_Info + "')");
                stinsert.executeUpdate("insert into T_EasyPay_No_Track (PAN, EasyPay_No, Barcode_Info) values ('" + PAN + "', '" + EasyPay_No + "', '" + Barcode_Info + "')");
            } catch (SQLException ex) {
                date = new Date();
                logger.fatal(dateFormat.format(date));
                logger.fatal("SQLException " + ex.getMessage());
                logger.fatal("With query: insert into T_EasyPay_No_Track (PAN, EasyPay_No, Barcode_Info) values ('" + PAN + "', '" + EasyPay_No + "', '" + Barcode_Info + "')");
                String TGResponseMessage = "Database Error";
            }
            try {
                Statement stinsert = Utils.TGDBcon.createStatement();
                date = new Date();
                logger.debug(dateFormat.format(date));
                logger.debug("Query: Update T_EasyPay_No_Track_New set Status = 'Assigned' where PAN = '" + PAN + "'");
                stinsert.executeUpdate("Update T_EasyPay_No_Track_New set Status = 'Assigned' where PAN = '" + PAN + "'");
                TGResponseCode = "00";
            } catch (SQLException ex) {
                date = new Date();
                logger.fatal(dateFormat.format(date));
                logger.fatal("SQLException " + ex.getMessage());
                logger.fatal("With query: Update T_EasyPay_No_Track_New set Status = 'Assigned' where PAN = '" + PAN + "'");
                String TGResponseMessage = "Database Error";
            }
            ...
            if (TGResponseCode.equals("00")) {
                out.println("					<h3>Card successfully issued</h3>");
            } else if (TGResponseCode.equals("01")) {
                out.println("					<h3>Card not successfully issued. Please try again.</h3>");
            }
