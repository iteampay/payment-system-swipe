...
public class MemberDetails extends HttpServlet {
    ...
    static Logger logger = Logger.getLogger(MemberDetails.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            out.println("<center>Access denied</center>");
            out.println("</body>");
            out.println("</html>");
            ...
            try {
                ...
                while (rsselect.next()) {
                    ...
                    out.println("<td><a href=\"/PaymentSystemSwipe/secureAdmin/MemberEdit.jsp?id=" + FormattedID + "\">" + Name + "</a></td>");
                    out.println("<td><a href=\"/PaymentSystemSwipe/secureAdmin/MemberEdit.jsp?id=" + FormattedID + "\">" + ID + "</a></td>");
                    out.println("<td><a href=\"/PaymentSystemSwipe/secureAdmin/MemberEdit.jsp?id=" + FormattedID + "\">" + MemNo + "</a></td>");
                    out.println("</tr>");
                }
                ...
                logger.fatal(dateFormat.format(date));
                logger.fatal("SQLException " + ex.getMessage());
                logger.fatal("Query:select FirstName + ' ' + SurName, Identity_No, Member_No from T_BP_Customers A, T_BP_Card B, T_BP_Card_Beneficiaries C where A.BP_Cust_ID = B.BP_Cust_ID and B.PAN = C.PAN and C.Beneficiary_No = '" + Beneficiary_No + "' and ((FirstName like '%' + '" + Search + "' + '%') or (SurName like '%' + '" + Search + "' + '%') or (Identity_No like '%' + '" + Search + "' + '%') or (Member_No like '%' + '" + Search + "' + '%')) order by " + OrderBy);
            }
            out.println("            <center><button type=\"button\" onclick =\"document.location.href = '/PaymentSystemSwipe/secureUser/home.jsp'\">Back to Main</button></center> <br />");
            out.println("<center><button type=\"button\" value=\"Back\" onclick=\"history.back()\">Back</button></center>");
